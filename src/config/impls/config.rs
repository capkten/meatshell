...
mod tests {
    ...
    fn session_notes_are_backward_compatible_across_field_rename() {
        let legacy: Session = serde_json::from_str(
            r#"{"id":"legacy","name":"Legacy","host":"example","port":22,"user":"root","auth":"password","note":"old note"}"#,
        )
        .unwrap();
        assert_eq!(legacy.notes, "old note");
        ...
        assert_eq!(
            encoded.get("notes").and_then(serde_json::Value::as_str),
            Some("line one\nline two")
        );
